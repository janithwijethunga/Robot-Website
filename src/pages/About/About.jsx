import React, { useEffect } from "react";
import Container from "../../components/layout/Container";
import Card from "../../components/ui/Card";
import SectionHeading from "../../components/sections/SectionHeading";
import { siteContent } from "../../content/siteContent";
import { setSEO } from "../../utils/seo";
import aboutDummy from "../../assets/about-dummy.svg";

export default function About() {
  const c = siteContent.pages.about;

  useEffect(() => {
    setSEO(c.seo, siteContent.brand.shortTitle);
  }, [c.seo]);

  return (
    <Container className="py-16 sm:py-20">
      <SectionHeading title={c.heading.title} subtitle={c.heading.subtitle} />
      <div className="mt-8 grid gap-6 lg:grid-cols-12 lg:items-start">
        <div className="lg:col-span-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {c.sections.map((s) => (
            <Card key={s.title}>
              <div className="font-semibold">{s.title}</div>
              <ul className="mt-3 space-y-2 text-sm text-neutral-600 dark:text-neutral-300">
                {s.bullets.map((b) => (
                  <li key={b} className="flex gap-2">
                    <span className="mt-2 h-1.5 w-1.5 rounded-full bg-primary-500" />
                    <span>{b}</span>
                  </li>
                ))}
              </ul>
            </Card>
          ))}
        </div>

        <div className="lg:col-span-5">
          <Card className="p-4">
            <div className="aspect-[4/3] overflow-hidden rounded-2xl">
              <img src={aboutDummy} alt="Project overview placeholder" className="h-full w-full object-cover" loading="lazy" />
            </div>
          </Card>
        </div>
      </div>
    </Container>
  );
}